  ...
  return (
    <div className="p-10 rounded shadow-lg shadow-teal-700">
      ...
      <Grid colCount={2} className="gap-4 md:gap-6 lg:gap-7 xl:gap-8 text-white text-[12px] md:text-[14px] lg:text-[15px] xl:text-[16px]">
        ...
        <div>
          <Text text={new Date(userData.dob).toLocaleDateString()} /> {/* Format date if needed */}
        </div>
        <div>
        ...
        <div>
          <Text text={userData.languages.map(lang => lang.language_name).join(", ")} /> {/* Display languages */}
        </div>
      </Grid>
    </div>
  );
